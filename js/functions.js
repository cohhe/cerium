...
jQuery(document).ready(function($) {
	...
	if ( jQuery('.frontpage-slider-wrapper').length ) {
		...
		var bxMainSlider = jQuery('.frontpage-slider-wrapper').bxSlider({
			...
			onSlideAfter: function($slideElement, oldIndex, newIndex) {
				...
				jQuery('.fp-slide-container').removeClass('reveal-content');

				jQuery($slideElement['0']).addClass('reveal-content');
		...
		jQuery('.frontpage-slider-wrapper .fp-slide-container').first().addClass('reveal-content');
	...
	jQuery('.main-menu-icon').on('click', function() {
		...
		jQuery('body').addClass('show-main-menu');

	});



	jQuery('.main-menu-close').on('click', function() {

		jQuery('body').removeClass('show-main-menu');

	});



	function cerium_isScrolledIntoView(elem) {
	...
	jQuery(document).scroll(function() {
		...
		jQuery('.nav-menu li a').removeClass('active-menu-item');

		jQuery('#entry-content-wrapper > div').each(function() {

			var content_div = jQuery(this);

			if ( cerium_isScrolledIntoView(content_div) && typeof content_div.attr('id') != 'undefined' ) {

				jQuery('.nav-menu li.menu-item-type-custom a').each(function() {

					if ( jQuery(this).attr('href') == '#'+content_div.attr('id') ) {
	...
	jQuery('li.menu-item-type-custom a').on('click', function(e) {
	...
	jQuery('.menu-item-has-children').on('click', function(e) {
		...
		jQuery(this).toggleClass('show-menu-submenu');
...
( function( $ ) {
	...
	if ( jQuery('[data-tagid]').length ) {

		jQuery('[data-tagid]').each(function() {

			jQuery(this).attr('id', jQuery(this).attr('data-tagid'));
	...
	( function() {
		...
		$( '.menu-toggle' ).on( 'click.cerium', function() {
	...
	_window.on( 'hashchange.cerium', function() {
	...
	$( function() {
		...
		if ( _window.width() > 781 ) {
			...
			if ( body.is( '.header-image' ) ) {
				...
				_window.on( 'scroll.cerium', function() {
		...
		$( '.primary-navigation, .secondary-navigation' ).find( 'a' ).on( 'focus.cerium blur.cerium', function() {
...
function cerium_checkbox( checkbox ) {

	checkbox.parent().append('<span class="cerium-checkbox"></span>');
