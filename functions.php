<?php
/**
 * Cerium 1.0 functions and definitions
 *
...
if ( ! function_exists( 'cerium_setup' ) ) :
	/**
	 * Cerium 1.0 setup.
	 *
	...
	function cerium_setup() {
		...
		/*
		 * Make Cerium 1.0 available for translation.
		 *
		 * Translations can be added to the /languages/ directory.
		 * If you're building a theme based on Cerium 1.0, use a find and
		 * replace to change 'cerium' to the name of your theme in all
		 * template files.
		 */
		load_theme_textdomain( 'cerium', get_template_directory() . '/languages' );
		...
		add_image_size( 'cerium-small-thumbnail', 70, 70, true );
		add_image_size( 'cerium-full-width', 1110, 831, true );
		add_image_size( 'cerium-thumbnail', 490, 318, true );
		add_image_size( 'cerium-thumbnail-large', 650, 411, true );
		add_image_size( 'cerium-medium-thumbnail', 350, 234, true );
		add_image_size( 'cerium-related-thumbnail', 255, 170, true );
		...
		add_theme_support( 'custom-background', apply_filters( 'cerium_custom_background_args', array(
...
function cerium_comment_count( $post_id ) {
 ...
 * @return void
 */
function cerium_content_width() {
	if ( is_attachment() && wp_attachment_is_image() ) {
		$GLOBALS['content_width'] = 810;
	}
}
add_action( 'template_redirect', 'cerium_content_width' );
 ...
 */
function cerium_get_featured_posts() {
	/**
	 * Filter the featured posts to return in Cerium 1.0.
	 *
	 * @since Cerium 1.0
	 *
	 * @param array|bool $posts Array of featured posts, otherwise false.
	 */
	return apply_filters( 'cerium_get_featured_posts', array() );
}
 ...
 */
function cerium_has_featured_posts() {
	return ! is_paged() && (bool) cerium_get_featured_posts();
}

/**
 * Register three Cerium 1.0 widget areas.
 *
 * @since Cerium 1.0
 *
 * @return void
...
add_action( 'widgets_init', 'cerium_widgets_init' );

/**
 * Register Lato Google font for Cerium 1.0.
 *
 * @since Cerium 1.0
 *
 * @return string
 */
function cerium_font_url() {
	$font_url = '';
	/*
...
function cerium_excerpt_length( $length ) {
	return 70;
}
add_filter( 'excerpt_length', 'cerium_excerpt_length', 999 );
	...
	if (!is_home() && !is_front_page()) {
		...
		if (get_query_var('paged')) {
			if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
				$output .= ' (';
			$output .= __('Page', 'cerium') . ' ' . get_query_var('paged');
			if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
				$output .= ')';
		}
 ...
 * @return void
 */
function cerium_scripts() {

	wp_enqueue_style( 'bootstrap', get_template_directory_uri() . '/css/bootstrap.css', array() );

	// Add Google fonts
	wp_register_style('googleFonts', '//fonts.googleapis.com/css?family=Raleway:300,400,600,700|Montserrat:300,400,600,700|Open+Sans:300,400,600,700&subset=latin');
	...
	wp_enqueue_style( 'cerium-style', get_stylesheet_uri(), array( 'genericons' ) );

	// Load the Internet Explorer specific stylesheet.
	wp_enqueue_style( 'cerium-ie', get_template_directory_uri() . '/css/ie.css', array( 'cerium-style', 'genericons' ), '20131205' );
	wp_style_add_data( 'cerium-ie', 'conditional', 'lt IE 9' );
...
// Admin Javascript
add_action( 'admin_enqueue_scripts', 'cerium_admin_scripts' );
function cerium_admin_scripts() {
	wp_register_script('master', get_template_directory_uri() . '/inc/js/admin-master.js', array('jquery'));
	wp_enqueue_script('master');
}

if ( ! function_exists( 'cerium_the_attached_image' ) ) :
	/**
	 ...
	 * @return void
	 */
	function cerium_the_attached_image() {
		$post                = get_post();
		/**
		 * Filter the default Cerium 1.0 attachment size.
		 *
		 * @since Cerium 1.0
		 *
		 ...
		 * }
		 */
		$attachment_size     = apply_filters( 'cerium_attachment_size', array( 810, 810 ) );
 ...
 */
function cerium_body_classes( $classes ) {
	global $post;
	$cerium_layout = '';
	...
	$cerium_layout = LONGFORM_LAYOUT;
	if ( !empty($cerium_layout) ) {
		$classes[] = $cerium_layout;
	}
	...
	if( isset($post->post_content) && has_shortcode( $post->post_content, 'cerium_main_slider' ) ) {
		$classes[] = 'cerium-main-slider';
	}

	return $classes;
}
add_filter( 'body_class', 'cerium_body_classes' );

/* Related posts */
function cerium_the_related_posts() {
	global $post;
						...
						<a rel="external" href="<?php the_permalink(); ?>">
							<div class="related-image">
								...
								if ( has_post_thumbnail() ) {
									the_post_thumbnail('cerium-medium-thumbnail');
								} else {
									echo '<img src="'.get_template_directory_uri().'/images/no-post-img.png" class="related-post-image" alt="'.__('Post without image', 'cerium').'">';
								}
								?>
								<span class="single-open-post-date"><?php echo human_time_diff(get_the_time('U',get_the_ID()),current_time('timestamp')) .  ' '.__('ago', 'cerium'); ?></span>
							</div>
						</a>
 ...
 */
class Cerium_Header_Menu_Walker extends Walker_Nav_Menu {
...
function vh_register_required_plugins() {
	...
	$config = array(
		...
		'strings'      		=> array(
			...
			'notice_can_activate_required'    			=> _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'cerium' ), // %1$s = plugin name(s)
			'notice_can_activate_recommended'			=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'cerium' ), // %1$s = plugin name(s)
			'notice_cannot_activate' 					=> _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'cerium' ), // %1$s = plugin name(s)
			'notice_ask_to_update' 						=> _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'cerium' ), // %1$s = plugin name(s)
